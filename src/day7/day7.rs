...
pub fn _run() {
    ...
    for line in lines {
        match line.split(" ").collect::<Vec<&str>>()[..] {
            ...
            ["$", "cd", dir] => {
                dirstack.push(dirstack.last().unwrap_or(&"".to_string()).to_owned() + "-" + dir)
            }
            ["$", "ls"] => {}
            ["dir", _] => {}
            [amount, _] => {
                for dir in dirstack.clone() {
                    dirsizes
                        .entry(dir)
                        .and_modify(|s| *s += amount.parse::<i32>().unwrap())
                        .or_insert(amount.parse::<i32>().unwrap());
                }
            }
            _ => {}
        }
