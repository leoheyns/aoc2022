...
pub fn _run() {
    ...
    let part1: usize = include_str!("input")
        .split("\n\n")
        .map(|pair| pair.split("\n").collect::<Vec<&str>>())
        .map(|ps| parse_na(ps[0]) <= parse_na(ps[1]))
        .enumerate()
        .filter(|(_, b)| *b)
        .map(|(i, _)| i + 1)
        .sum();
    ...
    input.push_str("\n[[2]]\n[[6]]");
    let mut nested_arrays = input
        .split("\n")
        .map(|s| parse_na(s))
        .collect::<Vec<NestedArray>>();
    nested_arrays.sort_unstable();

    let part2: usize = nested_arrays
        .iter()
        .enumerate()
        .filter(|(_, na)| **na == parse_na("[[2]]") || **na == parse_na("[[6]]"))
        .map(|(i, _)| i + 1)
        .product();
